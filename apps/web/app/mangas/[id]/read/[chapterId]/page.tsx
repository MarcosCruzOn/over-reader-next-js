...
async function getChapterData(mangaId: string, chapterNumber: string) {
	try {
		// Batemos na nossa nova rota exata!
...
export default async function MangaReaderPage({
	...
	// Passamos os dois parâmetros agora!
	const responseData = await getChapterData(resolvedParams.id, resolvedParams.chapterId)

	// 🔥 A BLINDAGEM DEFINITIVA:
	// Iniciamos com um array vazio e só preenchemos se a resposta for ESTRITAMENTE um Array.
	let pages: string[] = []

	if (Array.isArray(responseData)) {
		// Cenário 1: O backend retornou a lista de imagens direto [ "img1", "img2" ]
		pages = responseData
	} else if (responseData && Array.isArray(responseData.pages)) {
		// Cenário 2: O backend retornou um objeto contendo a lista { pages: [ "img1", "img2" ] }
		pages = responseData.pages
	}

	// Se não for um array válido, ou se estiver vazio, renderiza a tela amigável e PARA a execução do resto.
	if (pages.length === 0) {
		return (
	...
	// Lógica de navegação entre capítulos
